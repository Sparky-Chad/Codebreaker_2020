...
def compile_sigs():
    ...
    if not os.path.exists(PATH):
        ...
        with open(PATH, 'rb') as f:
            ...
            #signatures = [i for i in signatures if 'video' in i[0].lower()]
...
def check_file(filename):
    ...
    if results:
        ...
        for sig, desc, offset in results:
            s += ("[+] {0:<%ds} : {1:<%d} {2:<20s}" % (big_sig, big_desc)).format(sig, desc, "<- Offset: x{:x}".format(offset)) + '\n'
